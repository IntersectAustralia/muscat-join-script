...
def get_sources_join(marc_code):
    ...
    for i in range(len(sources_result)):
        ...
        marc_all = str(sources_result[i][14]).split('=') # marc data appears in 14th column
...
# DB connection
conn = MySQLdb.connect(host, user, pword, database)
cursor = conn.cursor()
...
source_partner_marc_code = {'people': 100, 'institutions': 852, 'canonic_techniques': 695, 'liturgical_feasts': 657, 'places': 'NULL', 'sources': 773, 'catalogues': 690, 'standard_terms': 650, 'standard_titles': 240, 'works': 930}
